...
uint64 gfxQueueCpuTimestamp = 0;
uint64 gfxQueueGpuTimestamp = 0;
uint64 gfxQueueFreq = 0;

uint64 computeQueueGpuTimestamp = 0;
uint64 computeQueueCpuTimestamp = 0;
uint64 computeQueueFreq = 0;

uint64 hiPrioComputeQueueCpuTimestamp = 0;
uint64 hiPrioComputeQueueGpuTimestamp = 0;
uint64 hiPrioComputeQueueFreq = 0;
...
static void InitWorkload(Workload& workload, const char* name, WorkloadType type, uint64 workloadIdx)
{
	...
	workload.TimestampsReadbackBuffer.Initialize(sizeof(uint64) * 2);

	D3D12_QUERY_HEAP_DESC queryHeapDesc;
	queryHeapDesc.Type = D3D12_QUERY_HEAP_TYPE_TIMESTAMP;
	queryHeapDesc.Count = 2;
	queryHeapDesc.NodeMask = 0;
	DXCall(DX12::Device->CreateQueryHeap(&queryHeapDesc, IID_PPV_ARGS(&workload.QueryHeap)));
...
static void ShutdownWorkload(Workload& workload)
{
	...
	workload.TimestampsReadbackBuffer.Shutdown();
	DX12::Release(workload.QueryHeap);
}
...
void OverlappedExecution::Initialize()
{
	...
	DXCall(DX12::GfxQueue->GetClockCalibration(&gfxQueueGpuTimestamp, &gfxQueueCpuTimestamp));
	DXCall(DX12::GfxQueue->GetTimestampFrequency(&gfxQueueFreq));

	{
		...
		DXCall(computeQueue->GetTimestampFrequency(&computeQueueFreq));
		DXCall(computeQueue->GetClockCalibration(&computeQueueGpuTimestamp, &computeQueueCpuTimestamp));

		queueDesc.Priority = D3D12_COMMAND_QUEUE_PRIORITY_HIGH;
		DXCall(DX12::Device->CreateCommandQueue(&queueDesc, IID_PPV_ARGS(&hiPriorityComputeQueue)));

		DXCall(hiPriorityComputeQueue->GetTimestampFrequency(&hiPrioComputeQueueFreq));
		DXCall(hiPriorityComputeQueue->GetClockCalibration(&hiPrioComputeQueueGpuTimestamp, &hiPrioComputeQueueCpuTimestamp));
...
static void TimeWorkloads(Fence * waitFence, Workload** workloads, uint64 numWorkloads)
{
	...
	float workTimeMinMs = 10000000000.f;
	...
	while (completionCount < numWorkloads)
	{
		for (uint64 workloadIdx = 0; workloadIdx < numWorkloads; ++workloadIdx)
		{
			...
			uint64 * TimestampsPtr = nullptr;

			if (AppSettings::UseQueries)
			{
				// just wait for finish
				waitFence->Wait(workload.LastUpdatedFrame + 1);

				TimestampsPtr = (uint64*)workload.TimestampsReadbackBuffer.Map();
			}
			...
			{
				uint64 started = TimestampsPtr[0];
				uint64 ended = TimestampsPtr[1];

				uint64 freq = 0;
				uint64 cpuTimestamp;
				uint64 gpuTimestamp;
				if (workload.Type == WorkloadType::ComputeQueue)
				{
					if (AppSettings::UseHiPriorityComputeQueue)
					{
						freq = hiPrioComputeQueueFreq;
						gpuTimestamp = hiPrioComputeQueueGpuTimestamp;
						cpuTimestamp = hiPrioComputeQueueCpuTimestamp;
					}
					else
					{
						freq = computeQueueFreq;
						gpuTimestamp = computeQueueGpuTimestamp;
						cpuTimestamp = computeQueueCpuTimestamp;
					}
				}
				else
				{
					freq = gfxQueueFreq;
					gpuTimestamp = gfxQueueGpuTimestamp;
					cpuTimestamp = gfxQueueCpuTimestamp;
				}

				float startGpuMs = 1000.f * (started - gpuTimestamp) / float(freq);
				float endGpuMs = 1000.f * (ended - gpuTimestamp) / float(freq);

				workTimeMinMs = std::min(workTimeMinMs, startGpuMs);

				workload.StartTime = float(startGpuMs);
				workload.EndTime = float(endGpuMs);

				++completionCount;
				workload.TimestampsReadbackBuffer.Unmap();
			}
		}
	}
	...
	if (AppSettings::UseQueries)
	{
		for (uint64 workloadIdx = 0; workloadIdx < numWorkloads; ++workloadIdx)
		{
			...
			workload.StartTime -= workTimeMinMs;
			workload.EndTime -= workTimeMinMs;
		}
	}
...
void OverlappedExecution::Update(const Timer& timer)
{
	...
	if (DX12::CurrentCPUFrame > 0)
	{
		...
		if (numEnabledWorkloads > 0)
			TimeWorkloads(&computeFence, enabledWorkloads, numEnabledWorkloads);
	}
...
void OverlappedExecution::DoComputeWorkload(ID3D12GraphicsCommandList* cmdList, Workload& workload)
{
	...
	if (AppSettings::UseQueries)
		cmdList->EndQuery(workload.QueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, 0);

	cmdList->Dispatch(DX12::DispatchSize(numWorkloadElements, 64), 1, 1);

	if (AppSettings::UseQueries)
	{
		cmdList->EndQuery(workload.QueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, 1);
		cmdList->ResolveQueryData(workload.QueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, 0, 2, workload.TimestampsReadbackBuffer.Resource, 0);
	}
